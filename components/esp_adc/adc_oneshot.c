...
static adc_oneshot_unit_ctx_t __adc_oneshot_temp_unit;
...
esp_err_t adc_oneshot_new_unit_static( const adc_oneshot_unit_init_cfg_t *init_config, adc_oneshot_unit_handle_t *ret_unit  )
{
    ...
    //ESP_GOTO_ON_FALSE(&__adc_oneshot_temp_unit, ESP_ERR_NO_MEM, err, TAG, "no mem for unit");
    ...
    s_ctx.units[init_config->unit_id] = &__adc_oneshot_temp_unit;
    _lock_release(&s_ctx.mutex);
    __adc_oneshot_temp_unit.unit_id = init_config->unit_id;
    __adc_oneshot_temp_unit.ulp_mode = init_config->ulp_mode;
    ...
    adc_oneshot_hal_init(&(__adc_oneshot_temp_unit.hal), &config);
    ...
    ESP_LOGD(TAG, "new adc unit%"PRId32" is created", __adc_oneshot_temp_unit.unit_id);
    *ret_unit = (adc_oneshot_unit_handle_t)&__adc_oneshot_temp_unit;
    return ESP_OK;
